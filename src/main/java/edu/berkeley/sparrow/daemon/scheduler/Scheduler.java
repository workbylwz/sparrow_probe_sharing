...
class requestElement {
	...
	public requestElement(String id, int num) {
		this.request_id = id;
		this.count = num;
	}
}
...
public class Scheduler {
	 ...
	 * {@code SparrowConf.CANCELLATION} is set to true.
	 */
	...
	private ConcurrentMap<String, requestElement> countMap;
	private ConcurrentLinkedQueue<String> idQueue;

	/**
	 * When a job includes SPREAD_EVENLY in the description and has this number
	 * of tasks, Sparrow spreads the tasks evenly over machines to evenly cache
	...
	public void initialize(Configuration conf, InetSocketAddress socket) throws IOException {
		...
		idQueue = Queues.newConcurrentLinkedQueue();
	 ...
	 * evenly across the cluster such that the input data for the query is
	 * triple replicated and spread evenly across the cluster.
	 *
	 * We signal that Sparrow should use this hack by adding SPREAD_TASKS to the
	 * job's description.
	 */
	private boolean isSpreadTasksJob(TSchedulingRequest request) {
		if ((request.getDescription() != null) && (request.getDescription().indexOf("SPREAD_EVENLY") != -1)) {
			...
			if (request.getTasks().size() != spreadEvenlyTaskSetSize) {
				LOG.debug("Not special case: job had " + request.getTasks().size() + " tasks rather than the expected "
						+ spreadEvenlyTaskSetSize);
				return false;
			}
	...
	public void handleJobSubmission(TSchedulingRequest request) throws TException {
		...
		for (TTaskSpec task : tasks) {
			constrained = constrained
					|| (task.preference != null && task.preference.nodes != null && !task.preference.nodes.isEmpty());
		}
		...
		countMap.put(requestId, new requestElement(requestId, request.getTasksSize()));
		idQueue.add(requestId);

		Map<InetSocketAddress, TEnqueueTaskReservationsRequest> enqueueTaskReservationsRequests;
		enqueueTaskReservationsRequests = taskPlacer.getEnqueueTaskReservationsRequests(request, requestId, backends,
				address);
		...
		for (Entry<InetSocketAddress, TEnqueueTaskReservationsRequest> entry : enqueueTaskReservationsRequests
			...
			try {
				...
				AUDIT_LOG.debug(Logging.auditEventString("scheduler_launch_enqueue_task", entry.getValue().requestId,
	...
	public List<TTaskLaunchSpec> getTask(String requestId, THostPort nodeMonitorAddress) {
		...
		requestElement peek = countMap.get(requestId);
                        AUDIT_LOG.info(Logging.auditEventString("!!!!!!!!!!!!! number1", requestId,
		...
		if (peek == null){
			...
			synchronized (peek) {
		        ...
		        AUDIT_LOG.info(Logging.auditEventString("!!!!!!!!!!!!! number2", requestId,
                                                nodeMonitorAddress.getHost()));
		synchronized (idQueue) {
			String headoflineId = idQueue.peek();
			TaskPlacer taskPlacer = requestTaskPlacers.get(headoflineId);

			if (taskPlacer == null) {
				LOG.debug("Received getTask() request for request " + headoflineId + ", which had no more "
						+ "unplaced tasks");
                         AUDIT_LOG.info(Logging.auditEventString("!!!!!!!!!!!!! number3", headoflineId,
                                                nodeMonitorAddress.getHost()));
				return Lists.newArrayList();
			}
			...
			if (taskLaunchSpecs == null || taskLaunchSpecs.size() > 1) {
				LOG.error(
						"Received invalid task placement for request " + headoflineId + ": " + taskLaunchSpecs.toString());
                                                        AUDIT_LOG.info(Logging.auditEventString("!!!!!!!!!!!!! number4", headoflineId,
                                                nodeMonitorAddress.getHost()));
				return Lists.newArrayList();
			} else if (taskLaunchSpecs.size() == 1) {
				AUDIT_LOG.info(Logging.auditEventString("scheduler_assigned_task", headoflineId,
						taskLaunchSpecs.get(0).taskId, nodeMonitorAddress.getHost()));
				String task_id = taskLaunchSpecs.get(0).getTaskId();
				task_id = task_id + "%" + headoflineId;
				taskLaunchSpecs.get(0).setTaskId(task_id);
			} else {
				AUDIT_LOG.info(Logging.auditEventString("scheduler_get_task_no_task", headoflineId,
						nodeMonitorAddress.getHost()));
			}

			if (taskPlacer.allTasksPlaced()) {
				LOG.debug("All tasks placed for request " + headoflineId);
				requestTaskPlacers.remove(headoflineId);
				idQueue.poll();
                        AUDIT_LOG.info(Logging.auditEventString("!!!!!!!!!!!!!all task placed", headoflineId,
                                                nodeMonitorAddress.getHost()));
	...
	/**
	 * Returns an ID that identifies a request uniquely (across all Sparrow
	 * schedulers).
	 *
	...
	private String getRequestId() {
		 ...
		 * scheduler followed by the counter. We use a counter rather than a
		 * hash of the request because there may be multiple requests to run an
		 * identical job.
		 */
