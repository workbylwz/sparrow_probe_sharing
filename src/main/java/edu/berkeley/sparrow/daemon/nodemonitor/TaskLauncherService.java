  ...
  private class TaskLaunchRunnable implements Runnable {
    ...
    public void run() {
      while (true) {
        ...
        String task_request = task.taskSpec.getTaskId();
        System.out.println(task_request.split("%")[0]+"!!!!!!!!!atention!!!!!!"+task_request.split("%")[1]);
        String task_id = task_request.split("%")[0];
        String request_new_id = task_request.split("%")[1];
        task.taskSpec.setTaskId(task_id);
        task.requestId = request_new_id;
...
AUDIT_LOG.debug(Logging.auditEventString("Error"+e.getMessage(), task.requestId,
          nodeMonitorInternalAddress.getHost()));
